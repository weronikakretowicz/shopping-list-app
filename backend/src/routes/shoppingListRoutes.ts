...
app.post('/lists', authMiddleware, validateRequest(createListSchema), async (c) => {
...
app.put('/lists/:id', authMiddleware, validateRequest(updateListSchema), async (c) => {
    ...
    const updatedList = await ShoppingList.findByIdAndUpdate(id, {name, items}, {new: true});
    if (!updatedList) {
